...
float pattern(vec2 p) {
  ...
  vec2 orbit = vec2(sin(phase * 1.7), cos(phase * 1.3)) * 0.65;
  vec2 shifted = p + orbit;
...
void main() {
  ...
  if (enableMouseInteraction == 1) {
    ...
    float distanceFromMouse = length(uv - mouseNDC);
    float mouseEffect = 1.0 - smoothstep(0.0, mouseRadius, distanceFromMouse);
...
function DitheredField({
  ...
  const pointer = useRef(new THREE.Vector2(-10_000, -10_000));
  ...
  useEffect(() => {
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      const pixelRatio = gl.getPixelRatio();
      pointer.current.set(
        (event.clientX - bounds.left) * pixelRatio,
        (event.clientY - bounds.top) * pixelRatio,
      );
    };
    const handlePointerLeave = () => pointer.current.set(-10_000, -10_000);
  ...
  useFrame((_state, delta) => {
    if (!disableAnimation) {
      ...
      const timeUniform = material.current?.uniforms.time;
      if (timeUniform) timeUniform.value = uniforms.time.value;
    }
    ...
    uniforms.mousePos.value.copy(pointer.current);
    const materialUniforms = material.current?.uniforms;
    const mouseUniform = materialUniforms?.mousePos;
    if (mouseUniform) mouseUniform.value.copy(pointer.current);
    const interactionUniform = materialUniforms?.enableMouseInteraction;
    if (interactionUniform) interactionUniform.value = enableMouseInteraction ? 1 : 0;
    const radiusUniform = materialUniforms?.mouseRadius;
    if (radiusUniform) radiusUniform.value = mouseRadius;
  });
